use super::*;
use crate::lcq::fp::{CompletionError, complete_multiply};
...
fn check(word: u32, first: u64, second: u64, fpcr: u32) -> EdgeKind {
    ...
    match exit.kind {
        EdgeKind::FpMultiply(operation) => {
            assert_eq!(actual, prestate);
            assert_eq!(exit.pc.get(), PC + 12);
            match complete_multiply(operation, &mut actual) {
                Ok(()) => {
    ...
    assert_eq!(
        actual, expected,
        "FMUL/FNMUL {word:08x}: {first:x}*{second:x}, FPCR {fpcr:x}"
    );
    exit.kind
}

#[test]
fn scalar_multiply_and_negation_match_rounding_special_inputs_and_traps() {
    for wide in [false, true] {
        ...
        for negate in [false, true] {
            let word = 0x1e22_0820 | ((wide as u32) << 22) | ((negate as u32) << 15);
            for (first, second) in [
                (bits(1.5), bits(1.0) + 1), // Directed rounding before negation.
                (bits(-1.5), bits(1.0) + 1),
                (bits(0.0), bits(-2.0)),
                (bits(-0.0), bits(-2.0)),
                (max, bits(2.0)),
                (bits(f64::INFINITY), bits(0.0)),
                (bits(f64::NAN), snan),
                (snan, bits(f64::NAN)),
                (1, bits(2.0)),
                (bits(2.0), 1),
                ...
                for fpcr in [1 << 8, 1 << 10, 1 << 12, (1 << 24) | (1 << 15)] {
                    check(word, first, second, fpcr);
                }
            }
            assert_eq!(
                check(word, bits(1.5), bits(2.0), 0),
                EdgeKind::Breakpoint(0)
            );
            for rd in [1, 2, 31] {
                check((word & !31) | rd, bits(1.5), bits(2.0), 0);
...
#[test]
fn scalar_multiply_tiny_results_match_arm_status_and_rounding() {
    for wide in [false, true] {
        let (minimum, half, sign) = if wide {
            (f64::MIN_POSITIVE.to_bits(), 0.5f64.to_bits(), 1 << 63)
        } else {
            (
        ...
        for negate in [false, true] {
            let word = 0x1e22_0820 | ((wide as u32) << 22) | ((negate as u32) << 15);
            for (first, second) in [
                (minimum, half),         // Exact tiny.
                (minimum + 1, half),     // Inexact tiny.
                (2 * minimum - 1, half), // May round up to a normal.
                (minimum, minimum),      // May round down to zero.
            ] {
                for mode in 0..8 {
                    // All rounding modes, with/without FZ.
                    for sign in [0, sign] {
...
#[test]
fn multiply_exact_boundary_preserves_pending_status_and_final_maps() {
    let words = [0x1e62_8823, 0x1e64_08a0, 0xd420_0000]; // FNMUL D3,D1,D2; FMUL D0,D5,D4
    let memory = memory(&words);
        ...
        let exact = lowered
            ...
            .find(|s| {
                s.exit.is_some_and(|e| {
                    e.pc.get() == PC + 4 && matches!(e.kind, EdgeKind::FpMultiply(_))
                })
            })
            .unwrap();
    ...
    let mut actual = A64State::default();
    actual.set_pc(PC);
    actual.set_vector(1, u128::from(1.5f64.to_bits()));
    actual.set_vector(2, u128::from(1.0f64.to_bits() + 1));
    actual.set_vector(5, u128::from(f64::INFINITY.to_bits()));
    let mut expected = actual.clone();
    execute_one(&TargetPlatform::Switch1, &mut expected, words[0]).unwrap();
    let (_, exit) = execute_memory(&memory, words.len(), &mut actual);
    assert_eq!(actual, expected);
    let EdgeKind::FpMultiply(operation) = exit.kind else {
        panic!("{exit:?}");
    };
    complete_multiply(operation, &mut actual).unwrap();
    execute_one(&TargetPlatform::Switch1, &mut expected, words[1]).unwrap();
    assert_eq!(actual, expected);
    assert_eq!(actual.fpsr(), 0x11); // Native IXC and exact IOC from infinity*zero.
}
