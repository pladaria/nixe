...
use nixe_cpu::execution::{CpuProcessId, CpuThreadId};
...
impl JitPerformanceReport {
    ...
    fn append_domain(
        ...
        domain: CpuProcessId,
        elapsed: Duration,
        executors: &[(CpuThreadId, ThreadPerformance)],
        ...
        let mut total = ThreadPerformance::default();
...
pub(crate) struct ProcessPerformance {
    report: Arc<JitPerformanceReport>,
    domain: CpuProcessId,
    started: Instant,
    executors: Mutex<Vec<(CpuThreadId, ThreadPerformance)>>,
}

impl ProcessPerformance {
    pub(crate) fn new(report: Arc<JitPerformanceReport>, domain: CpuProcessId) -> Self {
        Self {
            report,
            domain,
            started: Instant::now(),
    ...
    pub(crate) fn record_thread(&self, executor: CpuThreadId, counters: ThreadPerformance) {
...
pub(crate) struct ThreadPerformance {
...
fn render_counters(output: &mut String, counters: &ThreadPerformance) {
    macro_rules! field {
