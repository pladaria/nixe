...
use nixe_cpu::execution::{
    ArchitecturalTimer, ControlRequest, ControlSnapshot, CpuControl, VcpuEventState,
};
use nixe_cpu::{
...
use crate::cache::ProcessCodeCache;
use crate::compilation_pool::CompilationPoolHandle;
use crate::performance::ThreadPerformance;
...
pub(crate) struct NativeContext<'a> {
    ...
    pub(crate) control: &'a CpuControl,
    pub(crate) code_cache: Option<&'a ProcessCodeCache>,
    ...
    pub(crate) timer: &'a dyn ArchitecturalTimer,
    pub(crate) events: &'a VcpuEventState,
    pub(crate) performance: Option<&'a mut ThreadPerformance>,
}
...
impl<'a> NativeContext<'a> {
    ...
    pub(crate) fn with_performance(
        mut self,
        performance: Option<&'a mut ThreadPerformance>,
    ) -> Self {
...
unsafe extern "C" fn system(frame: *mut ExecutionFrame, operation: u32, argument: u64) -> u32 {
    match catch_unwind(AssertUnwindSafe(|| {
        ...
        if !snapshot.contains(ControlRequest::Preempt)
        ...
        frame.control.request_flags = u32::from(snapshot.contains(ControlRequest::Preempt))
            | (u32::from(snapshot.contains(ControlRequest::CodeInvalidation)) << 1);
