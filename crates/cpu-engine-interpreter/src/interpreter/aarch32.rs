...
pub(super) fn write_register(
    ...
    index: u8,
    value: u32,
) -> Result<SemanticControl, nixe_cpu::state::a32::InvalidBranchTarget> {
    if index == 15 {
...
pub(super) fn execute_multiply(
    state: &mut A32State,
    instruction: nixe_cpu::decode::aarch32::Multiply,
) -> Result<SemanticControl, nixe_cpu::state::a32::InvalidBranchTarget> {
    ...
    let control = if instruction.load {
        ...
        write_register(state, instruction.rt, value).map_err(|_| {
            DataAccessFault::new(
                ...
                nixe_cpu::memory::DataAccessFaultReason::Injected("invalid loaded PC".into()),
            )
        })?
    } else {
